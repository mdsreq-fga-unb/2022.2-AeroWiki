...
function ProjectsTable() {
  ...
  const onUpdate = async (
    { id, name, isfixed, editable, ongoing, active },
    action
  ) => {
    try {
      let response = await updateProject({
        id,
        name,
        isfixed,
        editable,
      ...
      <div className="projects-table">
        ...
        <div className="fixed-projects">
          {JSON.parse(sessionStorage.getItem("projectsData")).map(
            (item, index) => {
              if (
                item.area === sessionStorage.getItem("area") &&
                item.subarea === sessionStorage.getItem("subarea") &&
                item.active
              ) {
                setTimeout(function () {
                  var cargo = sessionStorage.getItem("cargo");
                  ...
                  if (!item.ongoing) {
                    document
                      .getElementById("projeto-" + index)
                      .classList.add("finalizado");
                  }
                  ...
                  if (cargo === "Membro") {
                    ...
                    if (item.isfixed) {
                      document
                        .getElementById("fix-" + index)
                        .classList.add("fixado");
                    } else {
                      document
                        .getElementById("fix-" + index)
                        .classList.remove("fixado");
                    }
                  }
                }, 3);
                ...
                return (
                  ...
                  >
                    <div
                      id={"projetostatus-" + index}
                      className="projetostatus"
                      onClick={() => classificarProjeto(item)}
                    >
                      <span
                        id="projetostatus-texto"
                    ...
                    <div id="project-text">
                      <div id="top">
                        ...
                        <div
                          id={"fix-" + index}
                          className="fixar-div"
                          onClick={() => fixarProjeto(item)}
                        >
                          <span className="project-areas-text">
                            Fixar Projeto
                          </span>
                          <FontAwesomeIcon
            ...
            (item, index) => {
              if (
                ...
                return (
                  <div
                    id={"projeto-" + index}
                    className="fixed-project projarquivado"
                    key={index}
                  >
                    <div
                      id="project-image"
